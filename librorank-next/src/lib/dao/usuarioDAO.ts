...
export function getTituloLector(totalLeidos: number): string {
  if (totalLeidos >= 200) return 'Oráculo Eterno'
  if (totalLeidos >= 150) return 'Sabio de los Libros'
  if (totalLeidos >= 100) return 'Guardián del Conocimiento'
  if (totalLeidos >= 75) return 'Erudito'
  if (totalLeidos >= 50) return 'Maestro Lector'
  if (totalLeidos >= 30) return 'Explorador Literario'
  if (totalLeidos >= 20) return 'Aventurero de Páginas'
  if (totalLeidos >= 10) return 'Lector Comprometido'
